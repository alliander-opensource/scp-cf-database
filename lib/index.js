...
class SapCfDatabase {
	...
	get Connection() {
		...
		if (!("connection" in mWM)) {
			...
			// exec omschrijven zodat drop er in zit
	...
	async execQuery(sQuery, aParameters, mOptions) { //es-lint-disable-line
		...
		if (sQuery.toUpperCase().startsWith("CALL") && ! Array.isArray(aParameters)) {
			...
			// const fnExec = async (sQuery, aParams) => {
			// 	const oStmt = await that.Connection.prepare(sQuery);
			// 	return new Promise((fnResolve, fnReject) => {
			// 		oStmt.exec(aParams, (err, mResult) => {
			// 			if (err) { return fnReject(oStmt); }	
			// 			oStmt.drop(err => {
			...
			// 	});
			// }
			// vResult = await fnExec(sQuery, aParams);
		}
	...
	static async query(sStatement, aParameters) {
		if (! _private.staticDB) { 
			...
			_private.staticDB.startRun			= async (...args) => {
				...
				try {
					const vResult = await _private.staticDB.execQuery.apply(_private.staticDB, args);  //eslint-disable-line
					fnOnDone(sRunId);
					return vResult;
				} catch(e) {
					fnOnDone(sRunId);
