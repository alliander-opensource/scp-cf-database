...
class SapCfDatabase {
	...
	get Connection() {
		...
		if (!("connection" in mWM)) {
			...
			oConnection.prepare			= promisify(oConnection.prepare.bind(oConnection));
			...
			oConnection.exec = async (sQuery, aParams) => { //eslint-disable-line
				const oStmt = await oConnection.prepare(sQuery);
			...
			oConnection.execBatch = async (sQuery, aParams) => { //eslint-disable-line
				const oStmt = await oConnection.prepare(sQuery);
				return new Promise((fnResolve, fnReject) => {
	...
	async execBatch(sQuery, aData, { autoDisconnect = true} = {}) { //es-lint-disable-line
        ...
        const vResult = await this.Connection.execBatch(sQuery, aValues);
        if (autoDisconnect) { this.Connection.disconnect(); }        
		return vResult;
    }
