...
class SapCfDatabase {
	...
	get Connection() {
		...
		if (!("connection" in mWM)) {
			...
			oConnection.connectionStart = (fnOrig => {
			...
			const fnPrepare			= promisify(oConnection.prepare.bind(oConnection));
	...
	async execQuery(sQuery, aParameters, mOptions) { //es-lint-disable-line
		...
		if (this.Connection.state() === "disconnected") {
			await this.Connection.connectionStart();
		}
		let vResult;
		if (sQuery.toUpperCase().startsWith("CALL") && ! Array.isArray(aParameters)) {
			...
			vResult = await this.Connection.connectionExec(sQuery, aParams);
		}
		if (mOpt.autoDisconnect) { this.Connection.connectionDisconnect(); }
		return vResult;
    }	

    /**
	 * Uitvoeren van een batch opdracht
	 */
	async execBatch(sQuery, aData, { autoDisconnect = true} = {}) { //es-lint-disable-line
		if (this.Connection.state() === "disconnected") {
			await this.Connection.connectionStart();
		}
		let	aValues = aData || [];
        if (! Array.isArray(aValues)) { aValues = [aValues]; }
        const vResult = await this.Connection.connectionExecBatch(sQuery, aValues);
        if (autoDisconnect) { this.Connection.connectionDisconnect(); }        
		return vResult;
    }
	 ...
	 */
	static async query(sStatement, aParameters, { queryBatch = false } = {}) {
			...
			const fnOnDone = sRunId => {
				...
				if (_private.staticDB.runIds.length === 0) {
					...
					_private.staticDB.timeoutRemoveDb = setTimeout(_ => {
						_private.staticDB.Connection.connectionDisconnect();
						_private.staticDB = null;
					}, 30000); // auto kill connection na 1/2 minuut inactiviteit
				}
			};
			_private.staticDB.startRun			= async (sStatement, aParameters, { autoDisconnect = false, queryBatch = false } = {}) => {
				if (_private.staticDB.timeoutRemoveDb) { clearTimeout(_private.staticDB.timeoutRemoveDb); }
				const sRunId = `_${Math.random().toString(36).substr(2, 9)}`;
				_private.staticDB.runIds.push(sRunId);
				try {
					const fnQueryMethod = (queryBatch) ? _private.staticDB.execBatch : _private.staticDB.execQuery ;
					const vResult = await fnQueryMethod.apply(_private.staticDB, [sStatement, aParameters, { autoDisconnect }]);  //eslint-disable-line
					fnOnDone(sRunId);
					return vResult;
				} catch(e) {
					fnOnDone(sRunId);
		...
		return _private.staticDB.startRun(sStatement, aParameters, { autoDisconnect: false, queryBatch }); 
	}
	
	static queryBatch(sStatement, aParameters) {
		return this.query(sStatement, aParameters, { queryBatch : true })
	}
	...
	static async callProcedure(sName, mData, vExportingParams) {
		...
		const
			...
			sExporting			= (sColumns && Array.isArray(aExportingParams) && aExportingParams.length) ? " ," + aExportingParams.join(" => ?, ") + " => ?" : "", //eslint-disable-line
		...
		if (mResult && typeof vExportingParams === "string") { 
			...
			return aExportingParams.reduce((a, b) => (a[b] = mResult[Object.keys(mResult).find(key => key.toLowerCase() === b.toLowerCase())]), {});
		} 
		return mResult;
	}
	...
	static insert(sTablename, vData) {
		if (Array.isArray(vData)) { // batchmode
			return Promise.all(vData.map(mData => _insertqueue.addInsert(sTablename, mData)));  //eslint-disable-line
		}
		return _insertqueue.addInsert(sTablename, vData); //eslint-disable-line
	}
	
	/**
	 * Insert een batch met nieuwe gegevens in de DB
	 * @param {string} sTablename - Tabel naam *schema name niet verplicht
	 * @param {array} array met dataa - data dat wordt toegevoegd aan de db [{ kolomNaam: "waarde", kolomNaam2: "Waarde 2" }, { kolomNaam: "waarde", kolomNaam2: "Waarde 2" }]
	 * @return {integer} affected rows
	 */
	static async insertBatch(sTableName, aBatchData) {
		if (!aBatchData || !aBatchData.length || typeof aBatchData[0] !== "object") {
			throw new Error("DB insertBatch error: Invalid batch data");
		}
		const aValues = [], 
			sColumns = Object.keys(aBatchData[0]).join(",");
		// batchdata controleren
		for(const i in aBatchData) { // for loop for performance
			aValues.push(Object.values(aBatchData[i]));		
			if (sColumns !== Object.keys(aBatchData[i]).join(",")) {
				throw new Error("DB insertBatch error: All batchdata should contain the same columns in the same order");
			}
		}
		return await this.queryBatch(
			`INSERT INTO "${sTableName}"("${Object.keys(aBatchData[0]).join('", "')}") VALUES (${_private.prepareParamsForStatement(Object.keys(aBatchData[0]))})`, 
			aValues
		);
	}
...
const _insertqueue	 = {
	...
	execute: async (sTablename, mData) => {
