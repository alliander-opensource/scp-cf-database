...
class SapCfDatabase {
	...
	constructor(mParams) {
		...
		_private.weakmap.set(this, {
			connOptions: {
               ...
               encrypt				: ("encrypt" in mParams) ? mParams.mParams : true,
	...
	get Connection() {
		...
		if (!("connection" in mWM)) {
			const oConnnection = hanaClient.createConnection();
			if (!oConnnection) { throw "Fout bij het opzetten van de connectie van de Database"; }
			// connect omzetten naar promise functie en automatisch connecteren met de ingestelde connOptions
			oConnnection.connect = (fnOrig => {
				return mConnectOptions => fnOrig(mConnectOptions || mWM.connOptions);
			})(oConnnection.connect.bind(oConnnection));
			// onderstaande functies allemaal omzetten naar een promise
			oConnnection.exec		= promisify(oConnnection.exec.bind(oConnnection));
			oConnnection.disconnect	= promisify(oConnnection.disconnect.bind(oConnnection));
			// resave weakmap
			mWM.connection = oConnnection;
	 ...
	 */
	async execQuery(sStatement, aParameters, mOptions) { //es-lint-disable-line
		...
		const vResult = await this.Connection.exec(sStatement, aParams);
	...
	static async callProcedure(sName, mData, vExportingParams) {
		...
		const
			...
			mResult 			= await this.query(`CALL "${sName}"(${sColumns} ${sExporting})`, Object.values(mData));
		...
		} else if (mResult && aExportingParams.length) {
